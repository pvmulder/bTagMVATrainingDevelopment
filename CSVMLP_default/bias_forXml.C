...
// define name Tree e dir_input in calcEntries()

void calcEntries(string  flavour, string category, vector<float> & entries);
...
int nIter =0;
 ...
 for(int j=0; j<3; j++){//loop on categories
   for(int i =0; i<3; i++){//loop on flavours
     ...
     calcEntries(flavour[i], cat[j], entries[nIter]);
...
for(int j=0; j<3; j++){//loop on categories	
   for(int k=1; k<3; k++){//loop on C and light
     ...
     for(int l = 0; l<15; l++ ){// loop on pt/eta bins defined in xml
       ...
       int indexb = k+j*3;
       float bias;
       bias = (float)((entries[index][l]/(entries[0][l]+entries[3][l]+entries[6][l]))/((entries[indexb][l]/(entries[k][l]+entries[k+3][l]+entries[k+6][l]))));
