...
std::vector<TH1F*> PlotLoader(string cat, string var, string norm, string etabin)
{
...
void FindMin(std::vector<TH1F*> plotted, double * Minimum, string var, int nHistos)
{
...
void Drawer(string cat, string var, string norm, string log,string config)
{	
...
//loading the plots
	...
	Histos_eta0 = PlotLoader(cat,var,norm,"_eta0to1p4");
	Histos_eta1 = PlotLoader(cat,var,norm,"_eta1p4to2p4");
...
//Drawing
	...
	if(config=="etaComp")
	{
		for(int i=0;i<Canvas.size();i++)
		{
			...
			FindMin(Plotted, &Min, var, Samples.at(i).size()-1);
			if(log=="log")
			{
...
//Saving Histos		
	...
	if(config=="etaComp")
	{
		outName.push_back("/home/descroix/MVA_Validation/plots/EtaBinComparisons/"+var+"_"+cat+"Vertex_"+log+"_"+norm+"Norm_EtaBinComp_B");
		outName.push_back("/home/descroix/MVA_Validation/plots/EtaBinComparisons/"+var+"_"+cat+"Vertex_"+log+"_"+norm+"Norm_EtaBinComp_C");
		outName.push_back("/home/descroix/MVA_Validation/plots/EtaBinComparisons/"+var+"_"+cat+"Vertex_"+log+"_"+norm+"Norm_EtaBinComp_DUSG");
	}
	if(config=="flavComp")
	{
		outName.push_back("/home/descroix/MVA_Validation/plots/EtaBinComparisons/"+var+"_"+cat+"Vertex_"+log+"_"+norm+"Norm_JetEtal1p4");
		outName.push_back("/home/descroix/MVA_Validation/plots/EtaBinComparisons/"+var+"_"+cat+"Vertex_"+log+"_"+norm+"Norm_JetEtam1p4");
	}
	string outFileName;
...
int main(int argc, char** argv)
{
	...
	string vertCat,scale,rep,etabin,conf;
...
//	conf = "etaComp";
	...
	Drawer(vertCat,"jetPt",scale,rep,conf);
	Drawer(vertCat,"jetEta",scale,rep,conf);
	Drawer(vertCat,"trackMultiplicity",scale,rep,conf);
	
	Drawer(vertCat,"trackSip2dSig",scale,rep,conf);
	Drawer(vertCat,"trackSip3dSig",scale,rep,conf);
	Drawer(vertCat,"trackSip3dSig_1stTrk",scale,rep,conf);
	Drawer(vertCat,"trackSip3dSig_2ndTrk",scale,rep,conf);
	Drawer(vertCat,"trackSip3dSig_3rdTrk",scale,rep,conf);
	Drawer(vertCat,"trackSip3dSig_restTrk",scale,rep,conf);
	Drawer(vertCat,"trackSip2dVal",scale,rep,conf);
	Drawer(vertCat,"trackSip3dVal",scale,rep,conf);
	
	Drawer(vertCat,"trackMomentum",scale,rep,conf);
	Drawer(vertCat,"trackEta",scale,rep,conf);
	Drawer(vertCat,"trackPtRel",scale,rep,conf);
	Drawer(vertCat,"trackPPar",scale,rep,conf);
	Drawer(vertCat,"trackEtaRel",scale,rep,conf);
	Drawer(vertCat,"trackDeltaR",scale,rep,conf);
	Drawer(vertCat,"trackPtRatio",scale,rep,conf);
	Drawer(vertCat,"trackPParRatio",scale,rep,conf);
	Drawer(vertCat,"trackJetDist",scale,rep,conf);
	Drawer(vertCat,"trackDecayLenVal",scale,rep,conf);
	Drawer(vertCat,"trackDecayLenSig",scale,rep,conf);
	Drawer(vertCat,"trackDecayLenSigTEST",scale,rep,conf);
	Drawer(vertCat,"trackSip2dSigAboveCharm",scale,rep,conf);
	Drawer(vertCat,"trackSip3dSigAboveCharm",scale,rep,conf);
	Drawer(vertCat,"trackSumJetEtRatio",scale,rep,conf);
	Drawer(vertCat,"trackSumJetDeltaR",scale,rep,conf);
	
	if(vertCat!="No")
	{
		Drawer(vertCat,"vertexMass",scale,rep,conf);
		Drawer(vertCat,"vertexNTracks",scale,rep,conf);
		Drawer(vertCat,"vertexEnergyRatio",scale,rep,conf);
		Drawer(vertCat,"vertexJetDeltaR",scale,rep,conf);
	}
	if((vertCat!="No")&&(vertCat!="Pseudo"))
	{	
		Drawer(vertCat,"jetNSecondaryVertices",scale,rep,conf);
		Drawer(vertCat,"flightDistance2dSig",scale,rep,conf);
		Drawer(vertCat,"flightDistance3dSig",scale,rep,conf);
		Drawer(vertCat,"flightDistance2dVal",scale,rep,conf);
		Drawer(vertCat,"flightDistance3dVal",scale,rep,conf);
	}
